...
import { ChevronDown, Package, Grid3X3 } from 'lucide-react';
              ...
              {categoriesLoading ? (
                <div className="p-8">
                  <div className="animate-pulse grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                    {[...Array(10)].map((_, i) => (
                      <div key={i} className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl p-4 h-32 flex flex-col items-center justify-center space-y-2">
                ...
                <div className="p-8">
                  ...
                  <div className="text-center mb-8">
                    <h2 className="text-2xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent mb-2">Shop by Category</h2>
                    ...
                    {categories.map((category: Category) => (
                      <div key={category._id} className="group relative">
                        ...
                        <Link 
                          ...
                          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white via-slate-50 to-slate-100/50 border border-slate-100 hover:border-primary-200 hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 p-4 h-32 flex flex-col items-center justify-center"
                        ...
                        >
                          ...
                          <div className="flex items-center justify-center mb-2">
                            <div className="w-12 h-12 rounded-xl overflow-hidden group-hover:scale-110 transition-transform duration-300 shadow-lg bg-gradient-to-br from-slate-100 to-slate-200">
                              {category.image?.url ? (
                                <img
                          ...
                          {/* Animated Border */}
                          <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-primary-400 via-primary-500 to-primary-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-full"></div>
                        </Link>
                            ...
                            <div className="space-y-1">
                              {subCategories[category._id].slice(0, 6).map((subCategory: SubCategory) => (
                                <Link
                                  ...
                                  className="flex items-center px-3 py-2.5 text-sm font-semibold text-slate-600 hover:text-primary-600 hover:bg-gradient-to-r hover:from-primary-50 hover:to-primary-100/50 rounded-xl transition-all duration-200 group"
                                  onClick={() => setIsOpen(false)}
                                >
                                  ...
                                  <div className="w-1 h-4 bg-gradient-to-b from-primary-400 to-primary-600 rounded-full opacity-0 group-hover:opacity-100 transition-opacity ml-2"></div>
                                </Link>
                              ))}
                              {subCategories[category._id].length > 6 && (
                                <Link
                                  to={`/category/${category._id}`}
                                  className="block px-3 py-2 text-xs text-primary-600 hover:bg-gradient-to-r hover:from-primary-50 hover:to-primary-100 rounded-xl transition-all font-medium border-t border-slate-100 mt-2 pt-3"
                                  onClick={() => setIsOpen(false)}
                                >
                  ...
                  <div className="mt-10 pt-8 border-t border-gradient-to-r from-transparent via-slate-200 to-transparent">
                    <div className="flex items-center justify-center">
                      <Link
                        to="/products"
                        className="inline-flex items-center space-x-3 py-4 px-8 bg-gradient-to-r from-primary-600 via-primary-700 to-primary-800 text-white rounded-2xl hover:shadow-2xl transform hover:-translate-y-1 hover:scale-105 transition-all duration-300 font-semibold relative overflow-hidden group"
                        onClick={() => setIsOpen(false)}
                      >
